...
interface Props {
  id: string
  initialNume: string
  initialVarsta: number
  initialJudet: string
  initialAdresa: string
  initialNevoi: string
  initialConditii: string
...
export default function EditSeniorForm({
  ...
  const [form, setForm] = useState({
    nume:     initialNume,
    varsta:   String(initialVarsta),
    judet:    initialJudet,
    adresa:   initialAdresa,
    nevoi:    initialNevoi,
    conditii: initialConditii,
  })
  ...
  // Debounced geocoding when address or county changes
  useEffect(() => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch(`/api/seniors/${id}`, {
      ...
      body: JSON.stringify({ ...form, varsta: Number(form.varsta), lat: coords?.lat ?? null, lng: coords?.lng ?? null }),
    })
    setLoading(false)
    if (!res.ok) {
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <p className="text-lg font-semibold text-gray-900">Modificările au fost salvate!</p>
        <p className="text-sm text-gray-500">Redirecționare la dashboard...</p>
      </div>
    )
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      {hasActiveContract && (
        <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-800">
          ...
          <p>
            <span className="font-medium">Atenție:</span> Modificarea adresei va actualiza raza de geofencing pentru îngrijitorul activ.
          </p>
        </div>
      )}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                value={form.varsta} onChange={set('varsta')} required
              />
            </div>
            <div>
